...
void display_usage()
{
    ...
    usage += "\n\tvroom [OPTION]... -i FILE\n";
    usage += "Options:\n";
    usage += "\t-a PROFILE:HOST (=" + vroom::DEFAULT_PROFILE + ":0.0.0.0)\t routing server\n";
    usage += "\t-p PROFILE:PORT (=" + vroom::DEFAULT_PROFILE + ":5000),\t routing server port\n";
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        cl_args.exploration_level = std::min(cl_args.exploration_level, cl_args.max_exploration_level);
    ...
    try {
        ...
        vroom::Solution sol = problem_instance.solve(cl_args.exploration_level, cl_args.nb_threads);
        ...
        vroom::io::write_to_json({error_code, e.message}, false, cl_args.output_file);
        exit(error_code);
