    ...
    namespace utils
    {
        ...
        inline Cost add_without_overflow(Cost a, Cost b)
        {
            if (a > std::numeric_limits<Cost>::max() - b) {
                throw Exception(ERROR::INPUT, "Too high cost values, stopping to avoid overflowing.");
            }
            return a + b;
        }
        ...
        inline Cost route_cost_for_vehicle(const Input& input, Index vehicle_rank, const std::vector<Index>& route)
        {
        ...
        inline Solution format_solution(const Input& input, const RawSolution& raw_routes)
        {
            const auto& m = input.get_matrix();
            ...
            for (std::size_t i = 0; i < raw_routes.size(); ++i) {
                ...
                for (std::size_t r = 0; r < route.size() - 1; ++r) {
                    ...
                    Duration travel = m[input.jobs[route[r]].index()][input.jobs[route[r + 1]].index()];
                    ETA += travel;
                    cost += travel;
                ...
                if (v.has_end()) {
                    ...
                    Duration travel = m[input.jobs[route.back()].index()][v.end.get().index()];
                    ETA += travel;
                    cost += travel;
            ...
            std::transform(unassigned_ranks.begin(), unassigned_ranks.end(), std::back_inserter(unassigned_jobs),
                           [&](auto j) { return input.jobs[j]; });

            return Solution(0, input.amount_size(), std::move(routes), std::move(unassigned_jobs));
        }

        inline Solution format_solution(const Input& input, const TWSolution& tw_routes)
        {
            const auto& m = input.get_matrix();
            ...
            for (std::size_t i = 0; i < tw_routes.size(); ++i) {
                ...
                for (std::size_t r = tw_r.route.size() - 1; r > 0; --r) {
                    ...
                    Duration diff = previous_job.service + m[previous_job.index()][current_job.index()];
                ...
                assert(steps.back().arrival + steps.back().waiting_time + steps.back().service -
                         steps.front().arrival ==
                       cost + service + forward_wt);

                routes.emplace_back(v.id, std::move(steps), cost, service, cost, forward_wt, amount);
            }
