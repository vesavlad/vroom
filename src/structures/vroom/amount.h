...
namespace vroom
{
    ...
    {
     public:
        Capacity    operator[](size_t i) const { return static_cast<const E&>(*this)[i]; };
    ...
    bool operator<<(const AmountExpression<E1>& lhs, const AmountExpression<E2>& rhs)
    {
    ...
    class Amount : public AmountExpression<Amount>
    {
     ...
     public:
        ...
        template <typename E>
        Amount(const AmountExpression<E>& u)
        {
            elems.resize(u.size());
        ...
        void push_back(Capacity c) { elems.push_back(c); };

        Capacity operator[](std::size_t i) const { return elems[i]; };

        Capacity& operator[](std::size_t i) { return elems[i]; };
    ...
    class AmountSum : public AmountExpression<AmountSum<E1, E2>>
    {
     ...
     public:
        AmountSum(const E1& a, const E2& b) : lhs(a), rhs(b) { assert(a.size() == b.size()); };

        Capacity operator[](std::size_t i) const { return lhs[i] + rhs[i]; };
    ...
    class AmountDiff : public AmountExpression<AmountDiff<E1, E2>>
    {
     ...
     public:
        AmountDiff(const E1& a, const E2& b) : lhs(a), rhs(b) { assert(a.size() == b.size()); };
