...
{
    Input::Input()
        : _start_loading(std::chrono::high_resolution_clock::now()), _no_addition_yet(true), _has_TW(false),
          _homogeneous_locations(true), _geometry(false), _all_locations_have_coords(true)
    {
    }

    void Input::set_geometry(bool geometry) { _geometry = geometry; }

    void Input::set_routing(std::unique_ptr<routing::Wrapper<Cost>> routing_wrapper)
    {
    ...
    void Input::add_job(const Job& job)
    {
        ...
        if (!current_job.location.user_index()) {
            ...
            if (search != _locations_to_index.end()) {
                ...
                _locations_to_index.insert(std::make_pair(current_job.location, new_index));
            }
        }
    ...
    void Input::add_vehicle(const Vehicle& vehicle)
    {
        ...
        if (vehicles.size() > 1) {
            _homogeneous_locations &= vehicles.front().has_same_locations(vehicles.back());
        }
    }
    ...
    void Input::check_amount_size(unsigned size)
    {
        if (_locations.empty()) {
            ...
            if (size != _amount_size) {
                throw Exception(ERROR::INPUT, "Inconsistent amount/capacity lengths: " + std::to_string(size) +
                                                " and " + std::to_string(_amount_size) + '.');
            }
        }
    }

    void Input::set_matrix(Matrix<Cost>&& m) { _matrix = std::move(m); }

    unsigned Input::amount_size() const { return _amount_size; }

    Amount Input::get_amount_lower_bound() const { return _amount_lower_bound; }

    bool Input::has_skills() const { return _has_skills; }

    bool Input::has_homogeneous_locations() const { return _homogeneous_locations; }
    ...
    const Matrix<Cost>& Input::get_matrix() const { return _matrix; }
    ...
    void Input::check_cost_bound() const
    {
        ...
        for (const auto& j : jobs) {
            jobs_departure_bound = utils::add_without_overflow(jobs_departure_bound, max_cost_per_line[j.index()]);
            jobs_arrival_bound   = utils::add_without_overflow(jobs_arrival_bound, max_cost_per_column[j.index()]);
        }
        ...
        for (const auto& v : vehicles) {
            if (v.has_start()) {
                start_bound = utils::add_without_overflow(start_bound, max_cost_per_line[v.start.get().index()]);
            }
            if (v.has_end()) {
                end_bound = utils::add_without_overflow(end_bound, max_cost_per_column[v.end.get().index()]);
            }
        }
    ...
    void Input::set_compatibility()
    {
        ...
        _vehicle_to_job_compatibility =
          std::vector<std::vector<bool>>(vehicles.size(), std::vector<bool>(jobs.size(), true));
        if (_has_skills) {
        ...
        _vehicle_to_vehicle_compatibility =
          std::vector<std::vector<bool>>(vehicles.size(), std::vector<bool>(vehicles.size(), false));
        for (std::size_t v1 = 0; v1 < vehicles.size(); ++v1) {
            ...
            for (std::size_t v2 = v1 + 1; v2 < vehicles.size(); ++v2) {
                for (std::size_t j = 0; j < jobs.size(); ++j) {
                    if (_vehicle_to_job_compatibility[v1][j] and _vehicle_to_job_compatibility[v2][j]) {
    ...
    Solution Input::solve(unsigned exploration_level, unsigned nb_thread)
    {
        if (_geometry and !_all_locations_have_coords) {
            ...
            throw Exception(ERROR::INPUT, "Route geometry request with missing coordinates.");
        }
        ...
        auto loading = std::chrono::duration_cast<std::chrono::milliseconds>(_end_loading - _start_loading).count();
        ...
        sol.summary.computing_times.solving =
          std::chrono::duration_cast<std::chrono::milliseconds>(_end_solving - _end_loading).count();
        ...
        if (_geometry) {
            ...
            auto routing = std::chrono::duration_cast<std::chrono::milliseconds>(_end_routing - _end_solving).count();
