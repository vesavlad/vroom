...
namespace vroom
{
    class TWRoute : public RawRoute
    {
     private:
        ...
        Duration new_earliest_candidate(const Input& input, Index job_rank, Index rank) const;
        Duration new_latest_candidate(const Input& input, Index job_rank, Index rank) const;
        ...
        bool is_fwd_valid_removal(const Input& input, const Index rank, const unsigned count) const;

        bool is_bwd_valid_removal(const Input& input, const Index rank, const unsigned count) const;
     ...
     public:
        ...
        std::size_t size() const { return route.size(); }

        // Check validity for addition of job at job_rank in current route
        // at rank.
        bool is_valid_addition_for_tw(const Input& input, const Index job_rank, const Index rank) const;
        ...
        bool is_valid_removal(const Input& input, const Index rank, const unsigned count) const;
