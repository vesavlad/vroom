...
namespace vroom
{
    TWRoute::TWRoute(const Input& input, Index i)
        : vehicle_rank(i), has_start(input.vehicles[i].has_start()), has_end(input.vehicles[i].has_end()),
          v_start(input.vehicles[i].tw.start), v_end(input.vehicles[i].tw.end)
    {
    }

    Duration TWRoute::new_earliest_candidate(const Input& input, Index job_rank, Index rank) const
    {
    ...
    Duration TWRoute::new_latest_candidate(const Input& input, Index job_rank, Index rank) const
    {
        const auto& m = input.get_matrix();
        const auto& v = input.vehicles[vehicle_rank];
        const auto& j = input.jobs[job_rank];
    ...
    void TWRoute::fwd_update_earliest_from(const Input& input, Index rank)
    {
        ...
        for (Index i = rank + 1; i < route.size(); ++i) {
            ...
            Duration    next_earliest = previous_earliest + previous_j.service + m[previous_j.index()][next_j.index()];
    ...
    void TWRoute::fwd_update_earliest_with_TW_from(const Input& input, Index rank)
    {
        ...
        for (Index i = rank + 1; i < route.size(); ++i) {
            const auto& previous_j = input.jobs[route[i - 1]];
            const auto& next_j     = input.jobs[route[i]];
            current_earliest += previous_j.service + m[previous_j.index()][next_j.index()];
    ...
    bool TWRoute::is_valid_addition_for_tw(const Input& input, const Index job_rank, const Index rank) const
    {
        const auto& m = input.get_matrix();
        const auto& v = input.vehicles[vehicle_rank];
        const auto& j = input.jobs[job_rank];
        ...
        if (valid) {
            ...
            auto overlap_candidate =
              std::find_if(j.tws.begin(), j.tws.end(), [&](const auto& tw) { return job_earliest <= tw.end; });
                                           ...
                                           const Index   last_rank) const
    {
        ...
        auto tw_candidate =
          std::find_if(first_j.tws.begin(), first_j.tws.end(), [&](const auto& tw) { return job_earliest <= tw.end; });
        ...
        while (next_job != last_job) {
            ...
            tw_candidate = std::find_if(next_j.tws.begin(), next_j.tws.end(),
                                        [&](const auto& tw) { return job_earliest <= tw.end; });
    ...
    void TWRoute::add(const Input& input, const Index job_rank, const Index rank)
    {
        ...
        auto candidate  = std::find_if(tws.begin(), tws.end(), [&](const auto& tw) { return job_earliest <= tw.end; });
        assert(candidate != tws.end());

        job_earliest = std::max(job_earliest, candidate->start);
        job_latest   = std::min(job_latest, candidate->end);

        tw_ranks.insert(tw_ranks.begin() + rank, std::distance(tws.begin(), candidate));
    ...
    bool TWRoute::is_fwd_valid_removal(const Input& input, const Index rank, const unsigned count) const
    {
        const auto& m = input.get_matrix();
        const auto& v = input.vehicles[vehicle_rank];
        ...
        if (current_rank == route.size()) {
            if (rank == 0 or !has_end) {
                ...
                return earliest[rank - 1] + new_last_job.service + m[new_last_job.index()][v.end.get().index()] <=
                       v_end;
            }
        }
        ...
        Duration job_earliest = previous_earliest + previous_service + previous_travel;
        ...
        while (current_rank < route.size()) {
            ...
            auto        candidate =
              std::find_if(tws.begin(), tws.end(), [&](const auto& tw) { return job_earliest <= tw.end; });
            assert(candidate != tws.end());
            job_earliest = std::max(job_earliest, candidate->start);
            job_earliest += current_job.service;
            if (current_rank < route.size() - 1) {
                job_earliest += m[current_job.index()][input.jobs[route[current_rank + 1]].index()];
            } else {
                if (has_end) {
    ...
    bool TWRoute::is_bwd_valid_removal(const Input& input, const Index rank, const unsigned count) const
    {
        const auto& m = input.get_matrix();
        const auto& v = input.vehicles[vehicle_rank];
        ...
        while (current_rank > 0) {
            ...
            assert(input.jobs[route[current_rank]].tws[tw_ranks[current_rank]].contains(next_latest));
    ...
    bool TWRoute::is_valid_removal(const Input& input, const Index rank, const unsigned count) const
    {
        assert(!route.empty());
        assert(rank + count <= route.size());

        return is_fwd_valid_removal(input, rank, count) and is_bwd_valid_removal(input, rank, count);
    }

    void TWRoute::remove(const Input& input, const Index rank, const unsigned count)
    {
        assert(rank + count <= route.size());
        ...
        if (!empty_route) {
            ...
            if (rank + count == route.size()) {
                ...
                if (has_end) {
                    auto gap = new_last_j.service + m[new_last_j.index()][v.end.get().index()];
                    assert(gap <= v_end);
                    end_latest -= gap;
                }
                          ...
                          const Index   last_rank)
    {
        ...
        while (first_job != last_job and insert_rank != last_rank) {
            ...
            if (insert_rank > 0) {
                ...
                current_earliest += previous_j.service + m[previous_j.index()][current_j.index()];
            }
            auto tw_candidate = std::find_if(current_j.tws.begin(), current_j.tws.end(),
        ...
        if (add_count < erase_count) {
            ...
            earliest.erase(earliest.begin() + insert_rank, earliest.begin() + insert_rank + to_erase);
            latest.erase(latest.begin() + insert_rank, latest.begin() + insert_rank + to_erase);
            tw_ranks.erase(tw_ranks.begin() + insert_rank, tw_ranks.begin() + insert_rank + to_erase);
        }
        ...
        if (!route.empty()) {
            ...
            if (insert_rank == route.size()) {
                ...
                latest[insert_rank] = std::min(end_latest, new_last_j.tws[tw_ranks[insert_rank]].end);
            } else {
    ...
    template bool TWRoute::is_valid_addition_for_tw(const Input&                 input,
