...
namespace vroom
{
    namespace utils
    {
        SolutionState::SolutionState(const Input& input)
            : _input(input), _m(_input.get_matrix()), _nb_vehicles(_input.vehicles.size()),
              _empty_amount(_input.amount_size()), fwd_amounts(_nb_vehicles), bwd_amounts(_nb_vehicles),
              fwd_costs(_nb_vehicles), bwd_costs(_nb_vehicles),
              fwd_skill_rank(_nb_vehicles, std::vector<Index>(_nb_vehicles)),
              bwd_skill_rank(_nb_vehicles, std::vector<Index>(_nb_vehicles)), edge_costs_around_node(_nb_vehicles),
              node_gains(_nb_vehicles), node_candidates(_nb_vehicles), edge_costs_around_edge(_nb_vehicles),
              edge_gains(_nb_vehicles), edge_candidates(_nb_vehicles),
              nearest_job_rank_in_routes_from(_nb_vehicles, std::vector<std::vector<Index>>(_nb_vehicles)),
              nearest_job_rank_in_routes_to(_nb_vehicles, std::vector<std::vector<Index>>(_nb_vehicles)),
        ...
        void SolutionState::setup(const RawSolution& sol)
        {
            ...
            Index x = 0;
            std::generate_n(std::inserter(unassigned, unassigned.end()), _input.jobs.size(), [&] { return x++; });
        ...
        void SolutionState::update_amounts(const std::vector<Index>& route, Index v)
        {
            ...
            std::transform(fwd_amounts[v].cbegin(), fwd_amounts[v].cend(), bwd_amounts[v].begin(), [&](const auto& a) {
        ...
        void SolutionState::update_skills(const std::vector<Index>& route, Index v1)
        {
            for (std::size_t v2 = 0; v2 < _nb_vehicles; ++v2) {
                ...
                auto fwd               = std::find_if_not(route.begin(), route.end(),
                                            [&](auto j_rank) { return _input.vehicle_ok_with_job(v2, j_rank); });
                fwd_skill_rank[v1][v2] = std::distance(route.begin(), fwd);

                auto bwd               = std::find_if_not(route.rbegin(), route.rend(),
                                            [&](auto j_rank) { return _input.vehicle_ok_with_job(v2, j_rank); });
        ...
        void SolutionState::update_nearest_job_rank_in_routes(const std::vector<Index>& route_1,
        ...
        void SolutionState::update_route_cost(const std::vector<Index>& route, Index v)
        {
