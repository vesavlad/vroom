    ...
    namespace routing
    {
                                                ...
                                                std::string                  extra_args = "") const
        {
            ...
            for (auto const& location : locations) {
                query += std::to_string(location.lon()) + "," + std::to_string(location.lat()) + "|";
            }
        ...
        std::string OrsHttpWrapper::send_then_receive(std::string query) const
        {
            ...
            try {
                ...
                throw Exception(ERROR::ROUTING, "Failed to connect to ORS at " + _server.host + ":" + _server.port);
            }
            return response;
        }

        Matrix<Cost> OrsHttpWrapper::get_matrix(const std::vector<Location>& locs) const
        {
            ...
            if (infos.HasMember("error")) {
                throw Exception(ERROR::ROUTING, "ORS matrix:" + std::string(infos["error"]["message"].GetString()));
            }
...
#endif
            if (infos.HasMember("error")) {
                throw Exception(ERROR::ROUTING, "ORS routes: " + std::string(infos["error"]["message"].GetString()));
            }
