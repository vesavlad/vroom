    ...
    namespace routing
    {
        ...
        std::string RoutedWrapper::send_then_receive(std::string query) const
        {
            ...
            try {
                ...
                throw Exception(ERROR::ROUTING, "Failed to connect to OSRM at " + _server.host + ":" + _server.port);
            }
            return response;
        }

        Matrix<Cost> RoutedWrapper::get_matrix(const std::vector<Location>& locs) const
        {
...
#endif
            if (infos["code"] != "Ok") {
                throw Exception(ERROR::ROUTING, "OSRM table: " + std::string(infos["message"].GetString()));
            }
...
#endif
            if (infos["code"] != "Ok") {
                throw Exception(ERROR::ROUTING, "OSRM route: " + std::string(infos["message"].GetString()));
            }
