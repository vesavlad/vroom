...
namespace vroom
{
    namespace routing
    {
        ...
        Matrix<Cost> LibosrmWrapper::get_matrix(const std::vector<Location>& locs) const
        {
            ...
            if (status == osrm::Status::Error) {
                throw Exception(ERROR::ROUTING, "libOSRM: " + result.values["code"].get<osrm::json::String>().value +
        ...
        void LibosrmWrapper::add_route_info(Route& route) const
        {
            ...
            for (auto& step : route.steps) {
                params.coordinates.emplace_back(osrm::util::FloatLongitude({step.location.lon()}),
            ...
            route.distance = round_cost(json_route.values["distance"].get<osrm::json::Number>().value);
            route.geometry = std::move(json_route.values["geometry"].get<osrm::json::String>().value);
