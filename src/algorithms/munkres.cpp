...
namespace vroom
{
    ...
    {
        template <class T>
        std::unordered_map<Index, Index> minimum_weight_perfect_matching(const Matrix<T>& m)
        {
        ...
        template <class T>
        std::unordered_map<Index, Index> greedy_symmetric_approx_mwpm(const Matrix<T>& m)
        {
            ...
            while (remaining_indices.size() > 0) {
                ...
                for (auto i = remaining_indices.begin(); i != remaining_indices.end(); ++i) {
                    auto j = i;
                    ++j;
        ...
        template std::unordered_map<Index, Index> minimum_weight_perfect_matching(const Matrix<Cost>& m);

        template std::unordered_map<Index, Index> greedy_symmetric_approx_mwpm(const Matrix<Cost>& m);
