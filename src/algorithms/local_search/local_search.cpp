...
namespace vroom
{
    namespace ls
    {
        ...
        LocalSearch<Route,
                    ...
                    IntraMixedExchange,
                    IntraRelocate,
                    IntraOrOpt>::LocalSearch(const Input& input, std::vector<Route>& sol, unsigned max_nb_jobs_removal)
            : _input(input), _matrix(_input.get_matrix()), _nb_vehicles(_input.vehicles.size()),
              _amount_lower_bound(_input.get_amount_lower_bound()),
              _double_amount_lower_bound(_amount_lower_bound + _amount_lower_bound),
              _max_nb_jobs_removal(max_nb_jobs_removal), _all_routes(_nb_vehicles), _sol_state(input), _sol(sol),
              _best_sol(sol)
        {
            ...
            Index v_rank     = 0;
            _best_cost       = std::accumulate(_sol.begin(), _sol.end(), 0, [&](auto sum, auto r) {
        ...
        void LocalSearch<Route,
                         ...
                         IntraMixedExchange,
                         IntraRelocate,
                         IntraOrOpt>::try_job_additions(const std::vector<Index>& routes, double regret_coeff)
        {
            bool job_added;
            ...
            do {
                ...
                for (const auto j : _sol_state.unassigned) {
                    ...
                    std::vector<Gain>  best_costs(routes.size(), std::numeric_limits<Gain>::max());
                        ...
                        if (_input.vehicle_ok_with_job(v, j) and
                            ...
                            for (std::size_t r = 0; r <= _sol[v].size(); ++r) {
                                if (_sol[v].is_valid_addition_for_tw(_input, j, r)) {
                                    Gain current_cost =
                                      utils::addition_cost(_input, _matrix, j, v_target, _sol[v].route, r);
                    ...
                    for (std::size_t i = 0; i < routes.size(); ++i) {
                        ...
                        double eval =
                          static_cast<double>(addition_cost) - regret_coeff * static_cast<double>(regret_cost);
                ...
                if (job_added) {
                    ...
                    auto        previous_cumul =
                      (best_rank == 0) ? Amount(_input.amount_size()) : best_fwd_amounts[best_rank - 1];
                    best_fwd_amounts.insert(best_fwd_amounts.begin() + best_rank, previous_cumul + job_amount);
                    std::for_each(best_fwd_amounts.begin() + best_rank + 1, best_fwd_amounts.end(),
                                  [&](auto& a) { a += job_amount; });
                    ...
                    for (std::size_t i = 0; i <= best_rank; ++i) {
                        _sol_state.bwd_amounts[best_route][i] = total_amount - _sol_state.fwd_amounts[best_route][i];
                    }
                         ...
                         IntraOrOpt>::run_ls_step()
        {
            ...
            std::vector<std::vector<Gain>> best_gains(_nb_vehicles, std::vector<Gain>(_nb_vehicles, 0));
            ...
            while (best_gain > 0) {
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size(); ++s_rank) {
                        for (unsigned t_rank = 0; t_rank < _sol[s_t.second].size(); ++t_rank) {
                            Exchange r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, _sol[s_t.second],
                                       s_t.second, t_rank);
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size() - 1; ++s_rank) {
                        for (unsigned t_rank = 0; t_rank < _sol[s_t.second].size() - 1; ++t_rank) {
                            CrossExchange r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, _sol[s_t.second],
                                            s_t.second, t_rank);
                            if (r.is_valid() and r.gain() > best_gains[s_t.first][s_t.second]) {
                                best_gains[s_t.first][s_t.second] = r.gain();
                                best_ops[s_t.first][s_t.second]   = std::make_unique<CrossExchange>(r);
                            }
                        }
                    }
                }
                ...
                for (const auto& s_t : s_t_pairs) {
                    if (s_t.first == s_t.second or _sol[s_t.first].size() == 0 or _sol[s_t.second].size() < 2) {
                        continue;
                    }

                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size(); ++s_rank) {
                        for (unsigned t_rank = 0; t_rank < _sol[s_t.second].size() - 1; ++t_rank) {
                            MixedExchange r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, _sol[s_t.second],
                                            s_t.second, t_rank);
                            if (r.is_valid() and r.gain() > best_gains[s_t.first][s_t.second]) {
                                best_gains[s_t.first][s_t.second] = r.gain();
                                best_ops[s_t.first][s_t.second]   = std::make_unique<MixedExchange>(r);
                            }
                        }
                    }
                }
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size(); ++s_rank) {
                        ...
                        for (int t_rank = _sol[s_t.second].size() - 1; t_rank >= 0; --t_rank) {
                            ...
                            TwoOpt r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, _sol[s_t.second],
                                     s_t.second, t_rank);
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size(); ++s_rank) {
                        ...
                        for (unsigned t_rank = 0; t_rank < _sol[s_t.second].size(); ++t_rank) {
                            ...
                            ReverseTwoOpt r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, _sol[s_t.second],
                                            s_t.second, t_rank);
                            if (r.gain() > best_gains[s_t.first][s_t.second] and r.is_valid()) {
                                best_gains[s_t.first][s_t.second] = r.gain();
                                best_ops[s_t.first][s_t.second]   = std::make_unique<ReverseTwoOpt>(r);
                            }
                        }
                    }
                }
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size(); ++s_rank) {
                        if (_sol_state.node_gains[s_t.first][s_rank] <= best_gains[s_t.first][s_t.second]) {
                            // Except if addition cost in route s_t.second is negative
                            // (!!), overall gain can't exceed current known best gain.
                            continue;
                        }
                        for (unsigned t_rank = 0; t_rank <= _sol[s_t.second].size(); ++t_rank) {
                            Relocate r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, _sol[s_t.second],
                                       s_t.second, t_rank);
                            if (r.is_valid() and r.gain() > best_gains[s_t.first][s_t.second]) {
                                best_gains[s_t.first][s_t.second] = r.gain();
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size() - 1; ++s_rank) {
                        if (_sol_state.edge_gains[s_t.first][s_rank] <= best_gains[s_t.first][s_t.second]) {
                            // Except if addition cost in route s_t.second is negative
                            // (!!), overall gain can't exceed current known best gain.
                            continue;
                        }
                        for (unsigned t_rank = 0; t_rank <= _sol[s_t.second].size(); ++t_rank) {
                            OrOpt r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, _sol[s_t.second],
                                    s_t.second, t_rank);
                            if (r.is_valid() and r.gain() > best_gains[s_t.first][s_t.second]) {
                                best_gains[s_t.first][s_t.second] = r.gain();
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size() - 2; ++s_rank) {
                        for (unsigned t_rank = s_rank + 2; t_rank < _sol[s_t.first].size(); ++t_rank) {
                            IntraExchange r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, t_rank);
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank <= _sol[s_t.first].size() - 4; ++s_rank) {
                        for (unsigned t_rank = s_rank + 3; t_rank < _sol[s_t.first].size() - 1; ++t_rank) {
                            IntraCrossExchange r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, t_rank);
                            if (r.is_valid() and r.gain() > best_gains[s_t.first][s_t.first]) {
                                best_gains[s_t.first][s_t.first] = r.gain();
                                best_ops[s_t.first][s_t.first]   = std::make_unique<IntraCrossExchange>(r);
                            }
                        }
                    }
                }
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size(); ++s_rank) {
                        for (unsigned t_rank = 0; t_rank < _sol[s_t.first].size() - 1; ++t_rank) {
                            if (t_rank <= s_rank + 1 and s_rank <= t_rank + 2) {
                                continue;
                            }
                            IntraMixedExchange r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, t_rank);
                            if (r.is_valid() and r.gain() > best_gains[s_t.first][s_t.first]) {
                                best_gains[s_t.first][s_t.first] = r.gain();
                                best_ops[s_t.first][s_t.first]   = std::make_unique<IntraMixedExchange>(r);
                            }
                        }
                    }
                }
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size(); ++s_rank) {
                        if (_sol_state.node_gains[s_t.first][s_rank] <= best_gains[s_t.first][s_t.first]) {
                        ...
                        for (unsigned t_rank = 0; t_rank <= _sol[s_t.first].size() - 1; ++t_rank) {
                            ...
                            IntraRelocate r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, t_rank);
                            if (r.gain() > best_gains[s_t.first][s_t.first] and r.is_valid()) {
                                best_gains[s_t.first][s_t.first] = r.gain();
                ...
                for (const auto& s_t : s_t_pairs) {
                    ...
                    for (unsigned s_rank = 0; s_rank < _sol[s_t.first].size() - 1; ++s_rank) {
                        ...
                        for (unsigned t_rank = 0; t_rank <= _sol[s_t.first].size() - 2; ++t_rank) {
                            if (t_rank == s_rank) {
                                continue;
                            }
                            IntraOrOpt r(_input, _sol_state, _sol[s_t.first], s_t.first, s_rank, t_rank);
                            if (r.is_valid() and r.gain() > best_gains[s_t.first][s_t.first]) {
                                best_gains[s_t.first][s_t.first] = r.gain();
                ...
                if (best_gain > 0) {
                    ...
                    auto update_candidates = best_ops[best_source][best_target]->update_candidates();

#ifndef NDEBUG
                    // Update route costs.
                    auto previous_cost =
                      std::accumulate(update_candidates.begin(), update_candidates.end(), 0,
                                      [&](auto sum, auto c) { return sum + _sol_state.route_costs[c]; });
                    for (auto v_rank : update_candidates) {
                        _sol_state.update_route_cost(_sol[v_rank].route, v_rank);
                    }
                    auto new_cost = std::accumulate(update_candidates.begin(), update_candidates.end(), 0,
                                                    [&](auto sum, auto c) { return sum + _sol_state.route_costs[c]; });
                    ...
                    try_job_additions(best_ops[best_source][best_target]->addition_candidates(), 0);
                         ...
                         IntraOrOpt>::run()
        {
            ...
            while (try_ls_step) {
                ...
                Index v_rank             = 0;
                Cost  current_cost       = std::accumulate(_sol.begin(), _sol.end(), 0, [&](auto sum, auto r) {
                    return sum + utils::route_cost_for_vehicle(_input, v_rank++, r.route);
                });

                bool solution_improved = (current_unassigned < _best_unassigned or
                ...
                if (solution_improved) {
                    ...
                    if (_best_unassigned < current_unassigned or
                        (_best_unassigned == current_unassigned and _best_cost < current_cost)) {
                         ...
                         IntraOrOpt>::remove_from_routes()
        {
            ...
            for (std::size_t v1 = 0; v1 < _nb_vehicles; ++v1) {
                for (std::size_t v2 = 0; v2 < _nb_vehicles; ++v2) {
                    ...
                    _sol_state.update_nearest_job_rank_in_routes(_sol[v1].route, _sol[v2].route, v1, v2);
                }
            }
            ...
            for (std::size_t v = 0; v < _sol.size(); ++v) {
                ...
                for (std::size_t r = 0; r < _sol[v].size(); ++r) {
                    ...
                    for (std::size_t other_v = 0; other_v < _sol.size(); ++other_v) {
                        if (other_v == v or !_input.vehicle_ok_with_job(other_v, _sol[v].route[r])) {
                            continue;
                        }
                        ...
                        if (_sol[other_v].size() != 0) {
                            auto nearest_from_rank  = _sol_state.nearest_job_rank_in_routes_from[v][other_v][r];
                            auto nearest_from_index = _input.jobs[_sol[other_v].route[nearest_from_rank]].index();
                            Gain cost_from          = _matrix[nearest_from_index][current_index];
                            best_relocate_distance  = std::min(best_relocate_distance, cost_from);

                            auto nearest_to_rank   = _sol_state.nearest_job_rank_in_routes_to[v][other_v][r];
                            auto nearest_to_index  = _input.jobs[_sol[other_v].route[nearest_to_rank]].index();
                                              ...
                                              IntraOrOpt>::indicators() const
        {
            ...
            si.cost       = _best_cost;
            si.used_vehicles =
              std::count_if(_best_sol.begin(), _best_sol.end(), [](const auto& r) { return !r.empty(); });
            return si;
        }
