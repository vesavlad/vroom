...
namespace vroom
{
    namespace heuristics
    {
        ...
        T basic(const Input& input, INIT init, float lambda)
        {
            ...
            // capacities ties.
            std::stable_sort(vehicles_ranks.begin(), vehicles_ranks.end(), [&](const auto lhs, const auto rhs) {
                auto& v_lhs = input.vehicles[lhs];
                auto& v_rhs = input.vehicles[rhs];
                return v_rhs.capacity << v_lhs.capacity or
                       (v_lhs.capacity == v_rhs.capacity and v_lhs.tw.length > v_rhs.tw.length);
            });
            ...
            for (Index v = 0; v < input.vehicles.size(); ++v) {
                ...
                if (init != INIT::NONE) {
                    ...
                    for (const auto job_rank : unassigned) {
                        ...
                        if (init == INIT::HIGHER_AMOUNT and higher_amount << input.jobs[job_rank].amount) {
                ...
                while (keep_going) {
                    ...
                    for (const auto job_rank : unassigned) {
                        ...
                        for (Index r = 0; r <= current_r.size(); ++r) {
                            float current_add = utils::addition_cost(input, m, job_rank, vehicle, current_r.route, r);

                            float current_cost = current_add - lambda * static_cast<float>(costs[job_rank]);

                            if (current_cost < best_cost and current_r.is_valid_addition_for_tw(input, job_rank, r)) {
        ...
        T dynamic_vehicle_choice(const Input& input, INIT init, float lambda)
        {
            ...
            std::vector<std::vector<Cost>> costs(input.jobs.size(), std::vector<Cost>(input.vehicles.size()));
            for (std::size_t j = 0; j < input.jobs.size(); ++j) {
                Index j_index = input.jobs[j].index();
            ...
            while (!vehicles_ranks.empty() and !unassigned.empty()) {
                ...
                std::vector<Cost> jobs_min_costs(input.jobs.size(), std::numeric_limits<Cost>::max());
                std::vector<Cost> jobs_second_min_costs(input.jobs.size(), std::numeric_limits<Cost>::max());
                for (const auto job_rank : unassigned) {
                ...
                const auto chosen_vehicle =
                  std::min_element(vehicles_ranks.begin(), vehicles_ranks.end(), [&](const auto lhs, const auto rhs) {
                      auto& v_lhs = input.vehicles[lhs];
                      auto& v_rhs = input.vehicles[rhs];
                      return closest_jobs_count[lhs] > closest_jobs_count[rhs] or
                             (closest_jobs_count[lhs] == closest_jobs_count[rhs] and
                ...
                // vehicles.
                std::vector<Cost> regrets(input.jobs.size(), std::numeric_limits<Cost>::max());
                for (const auto job_rank : unassigned) {
                ...
                if (init != INIT::NONE) {
                    ...
                    for (const auto job_rank : unassigned) {
                        ...
                        if (init == INIT::FURTHEST and furthest_cost < costs[job_rank][v_rank]) {
                ...
                while (keep_going) {
                    ...
                    for (const auto job_rank : unassigned) {
                        ...
                        for (Index r = 0; r <= current_r.size(); ++r) {
                            ...
                            float current_cost = current_add - lambda * static_cast<float>(regrets[job_rank]);
        ...
        template RawSolution dynamic_vehicle_choice(const Input& input, INIT init, float lambda);

        template TWSolution basic(const Input& input, INIT init, float lambda);

        template TWSolution dynamic_vehicle_choice(const Input& input, INIT init, float lambda);
