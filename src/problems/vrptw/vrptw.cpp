...
namespace vroom
{
    ...
    {
        auto parameters = (_input.has_homogeneous_locations()) ? homogeneous_parameters : heterogeneous_parameters;
        ...
        std::vector<std::vector<std::size_t>> thread_ranks(nb_threads, std::vector<std::size_t>());
            ...
            for (auto rank : param_ranks) {
                ...
                switch (p.heuristic) {
                    case HEURISTIC::BASIC:
                        tw_solutions[rank] = heuristics::basic<TWSolution>(_input, p.init, p.regret_coeff);
                        break;
                    case HEURISTIC::DYNAMIC:
                        tw_solutions[rank] =
                          heuristics::dynamic_vehicle_choice<TWSolution>(_input, p.init, p.regret_coeff);
                        break;
                }
        ...
        auto best_indic = std::min_element(sol_indicators.cbegin(), sol_indicators.cend());

        return utils::format_solution(_input, tw_solutions[std::distance(sol_indicators.cbegin(), best_indic)]);
    }
