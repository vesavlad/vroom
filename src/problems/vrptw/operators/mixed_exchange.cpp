...
namespace vroom
{
    namespace vrptw
    {
        ...
        bool MixedExchange::is_valid()
        {
            ...
            valid &= _tw_t_route.is_valid_addition_for_tw(_input, s_route.begin() + s_rank,
                                                          s_route.begin() + s_rank + 1, t_rank, t_rank + 1);

            if (valid) {
                // Keep target edge direction when inserting in source route.
                auto t_start = t_route.begin() + t_rank;
                _s_is_normal_valid =
                  _tw_s_route.is_valid_addition_for_tw(_input, t_start, t_start + 2, s_rank, s_rank + 1);
        ...
        void MixedExchange::apply()
        {
            ...
            _tw_s_route.replace(_input, t_job_ranks.begin(), t_job_ranks.end(), s_rank, s_rank + 1);

            _tw_t_route.replace(_input, s_job_ranks.begin(), s_job_ranks.end(), t_rank, t_rank + 2);
        }
