    ...
    namespace vrptw
    {
        CrossExchange::CrossExchange(const Input&                input,
            ...
            : cvrp::CrossExchange(input,
                                  ...
                                  t_vehicle,
                                  t_rank),
              _tw_s_route(tw_s_route), _tw_t_route(tw_t_route), _s_is_normal_valid(false), _s_is_reverse_valid(false),
              _t_is_normal_valid(false), _t_is_reverse_valid(false)
        {
        }
        ...
        bool CrossExchange::is_valid()
        {
            ...
            if (valid) {
                ...
                _s_is_normal_valid =
                  _tw_s_route.is_valid_addition_for_tw(_input, t_start, t_start + 2, s_rank, s_rank + 2);
                // Reverse target edge direction when inserting in source route.
                auto t_reverse_start = t_route.rbegin() + t_route.size() - 2 - t_rank;
                _s_is_reverse_valid = _tw_s_route.is_valid_addition_for_tw(_input, t_reverse_start, t_reverse_start + 2,
                                                                           s_rank, s_rank + 2);
            ...
            if (valid) {
                ...
                _t_is_normal_valid =
                  _tw_t_route.is_valid_addition_for_tw(_input, s_start, s_start + 2, t_rank, t_rank + 2);
                // Reverse source edge direction when inserting in target route.
                auto s_reverse_start = s_route.rbegin() + s_route.size() - 2 - s_rank;
                _t_is_reverse_valid = _tw_t_route.is_valid_addition_for_tw(_input, s_reverse_start, s_reverse_start + 2,
                                                                           t_rank, t_rank + 2);
        ...
        void CrossExchange::apply()
        {
            ...
            if (!reverse_t_edge) {
                ...
                auto t_reverse_start = t_route.rbegin() + t_route.size() - 2 - t_rank;
                t_job_ranks.insert(t_job_ranks.begin(), t_reverse_start, t_reverse_start + 2);
            }

            if (!reverse_s_edge) {
                _tw_t_route.replace(_input, s_route.begin() + s_rank, s_route.begin() + s_rank + 2, t_rank, t_rank + 2);
            } else {
                auto s_reverse_start = s_route.rbegin() + s_route.size() - 2 - s_rank;
                _tw_t_route.replace(_input, s_reverse_start, s_reverse_start + 2, t_rank, t_rank + 2);
            }

            _tw_s_route.replace(_input, t_job_ranks.begin(), t_job_ranks.end(), s_rank, s_rank + 2);
        }
