...
namespace vroom
{
    TSP::TSP(const Input& input, std::vector<Index> job_ranks, Index vehicle_rank)
        : VRP(input), _vehicle_rank(vehicle_rank), _job_ranks(std::move(job_ranks)), _is_symmetric(true),
          _has_start(_input.vehicles[_vehicle_rank].has_start()), _has_end(_input.vehicles[_vehicle_rank].has_end())
    {
        assert(!_job_ranks.empty());

        // Pick ranks to select from input matrix.
        std::vector<Index> matrix_ranks;
        std::transform(_job_ranks.cbegin(), _job_ranks.cend(), std::back_inserter(matrix_ranks),
    ...
    RawSolution TSP::raw_solve(unsigned, unsigned nb_threads) const
    {
        ...
        // local minima.
        tsp::LocalSearch sym_ls(_symmetrized_matrix, std::make_pair(!_round_trip and _has_start and _has_end, _start),
                                christo_sol, nb_threads);
        ...
        } while ((sym_two_opt_gain > 0) or (sym_relocate_gain > 0) or (sym_or_opt_gain > 0));
        ...
        if (!_is_symmetric) {
            ...
            tsp::LocalSearch asym_ls(_matrix, std::make_pair(!_round_trip and _has_start and _has_end, _start),
                                     (direct_cost <= reverse_cost) ? current_sol : reverse_current_sol, nb_threads);
            ...
            } while ((asym_two_opt_gain > 0) or (asym_relocate_gain > 0) or (asym_or_opt_gain > 0) or
                     (asym_avoid_loops_gain > 0));
        ...
        std::transform(current_sol.cbegin(), current_sol.cend(), std::back_inserter(init_ranks_sol),
