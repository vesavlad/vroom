...
namespace vroom
{
    namespace tsp
    {
        LocalSearch::LocalSearch(const Matrix<Cost>&     matrix,
                                 ...
                                 unsigned                nb_threads)
            : _matrix(matrix), _avoid_start_relocate(avoid_start_relocate), _edges(_matrix.size()),
              _nb_threads(std::min(nb_threads, static_cast<unsigned>(tour.size()))), _rank_limits(_nb_threads)
        {
            ...
            if (_nb_threads > 1) {
                ...
                std::partial_sum(number_of_lookups.begin(), number_of_lookups.end(),
        ...
        Cost LocalSearch::relocate_step()
        {
            ...
            auto look_up = [&](Index start, Index end, Cost& best_gain, Index& best_edge_1_start,
                ...
                for (Index edge_1_start = start; edge_1_start < end; ++edge_1_start) {
                    ...
                    while (edge_2_start != edge_1_start) {
                        ...
                        Cost  before_cost = edge_1_weight + edge_1_end_next_weight + _matrix[edge_2_start][edge_2_end];
                        Cost  after_cost =
                          first_potential_add + _matrix[edge_2_start][edge_1_end] + _matrix[edge_1_end][edge_2_end];
            ...
            for (std::size_t i = 0; i < _nb_threads - 1; ++i) {
                threads.emplace_back(look_up, _rank_limits[i], _rank_limits[i + 1], std::ref(best_gains[i]),
                                     std::ref(best_edge_1_starts[i]), std::ref(best_edge_2_starts[i]));
            }

            look_up(_rank_limits[_nb_threads - 1], _rank_limits[_nb_threads], std::ref(best_gains[_nb_threads - 1]),
                    std::ref(best_edge_1_starts[_nb_threads - 1]), std::ref(best_edge_2_starts[_nb_threads - 1]));
            ...
            auto  best_rank = std::distance(best_gains.begin(), std::max_element(best_gains.begin(), best_gains.end()));
        ...
        Cost LocalSearch::avoid_loop_step()
        {
            ...
            do {
                ...
                if (!_avoid_start_relocate.first or candidate != _avoid_start_relocate.second) {
                    while ((current != previous_candidate) and !candidate_relocatable) {
                        Index next = _edges[current];
                        if ((_matrix[current][candidate] + _matrix[candidate][next] <= _matrix[current][next]) and
                            (_matrix[current][candidate] > 0) and (_matrix[candidate][next] > 0)) {
            ...
            std::sort(relocatable_chains.begin(), relocatable_chains.end(),
                      [](const std::list<Index>& lhs, const std::list<Index>& rhs) { return lhs.size() > rhs.size(); });
            ...
            for (auto const& chain : relocatable_chains) {
                ...
                for (auto const& step : chain) {
                    ...
                    before_cost += _matrix[possible_position.at(step)][edges_c.at(possible_position.at(step))];
        ...
        Cost LocalSearch::two_opt_step()
        {
            ...
            auto look_up = [&](Index start, Index end, Cost& best_gain, Index& best_edge_1_start,
                ...
                for (Index edge_1_start = start; edge_1_start < end; ++edge_1_start) {
                    Index edge_1_end = _edges[edge_1_start];
                    for (Index edge_2_start = edge_1_start + 1; edge_2_start < _edges.size(); ++edge_2_start) {
                        ...
                        Cost before_cost = _matrix[edge_1_start][edge_1_end] + _matrix[edge_2_start][edge_2_end];
                        Cost after_cost  = _matrix[edge_1_start][edge_2_start] + _matrix[edge_1_end][edge_2_end];
            ...
            std::vector<std::thread> threads;
            for (std::size_t i = 0; i < _nb_threads - 1; ++i) {
                threads.emplace_back(look_up, _sym_two_opt_rank_limits[i], _sym_two_opt_rank_limits[i + 1],
                                     std::ref(best_gains[i]), std::ref(best_edge_1_starts[i]),
                                     std::ref(best_edge_2_starts[i]));
            }

            look_up(_sym_two_opt_rank_limits[_nb_threads - 1], _sym_two_opt_rank_limits[_nb_threads],
                    std::ref(best_gains[_nb_threads - 1]), std::ref(best_edge_1_starts[_nb_threads - 1]),
            ...
            if (best_gain > 0) {
                ...
                for (Index current = best_edge_1_end; current != best_edge_2_start; current = _edges[current]) {
        ...
        Cost LocalSearch::asym_two_opt_step()
        {
            ...
            std::vector<std::thread> threads;
            for (std::size_t i = 0; i < _nb_threads - 1; ++i) {
                threads.emplace_back(look_up, limit_nodes[i], limit_nodes[i + 1], std::ref(best_gains[i]),
                                     std::ref(best_edge_1_starts[i]), std::ref(best_edge_2_starts[i]));
            }

            look_up(limit_nodes[_nb_threads - 1], limit_nodes[_nb_threads], std::ref(best_gains[_nb_threads - 1]),
                    std::ref(best_edge_1_starts[_nb_threads - 1]), std::ref(best_edge_2_starts[_nb_threads - 1]));
        ...
        Cost LocalSearch::or_opt_step()
        {
            ...
            auto look_up = [&](Index start, Index end, Cost& best_gain, Index& best_edge_1_start,
                ...
                for (Index edge_1_start = start; edge_1_start < end; ++edge_1_start) {
                    ...
                    while (edge_2_start != edge_1_start) {
                        Index edge_2_end  = _edges[edge_2_start];
                        Cost  before_cost = edge_1_weight + next_next_2_weight + _matrix[edge_2_start][edge_2_end];
                        Cost  after_cost =
                          first_potential_add + _matrix[edge_2_start][edge_1_end] + _matrix[next][edge_2_end];
                        if (before_cost > after_cost) {
                            Cost gain = before_cost - after_cost;
                            if (gain > best_gain) {
                                best_gain         = gain;
