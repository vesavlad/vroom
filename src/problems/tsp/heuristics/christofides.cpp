...
namespace vroom
{
    namespace tsp
    {
        std::list<Index> christofides(const Matrix<Cost>& sym_matrix)
        {
            ...
            std::unordered_map<Index, std::list<Index>> adjacency_list = mst_graph.get_adjacency_list();
            ...
            std::unordered_map<Index, Index> mwpm = utils::minimum_weight_perfect_matching(sub_matrix);
            ...
            for (const auto& edge : mwpm) {
                if (mwpm.at(edge.second) == edge.first) {
                    mwpm_final.emplace(std::min(edge.first, edge.second), std::max(edge.first, edge.second));
                    ++total_ok;
                } else {
            ...
            if (!wrong_vertices.empty()) {
                std::unordered_map<Index, Index> remaining_greedy_mwpm =
                  utils::greedy_symmetric_approx_mwpm(sub_matrix.get_sub_matrix(wrong_vertices));

                // Adding edges obtained with greedy algo for the missing vertices
                // in mwpm_final.
                for (const auto& edge : remaining_greedy_mwpm) {
                    mwpm_final.emplace(std::min(wrong_vertices[edge.first], wrong_vertices[edge.second]),
                                       std::max(wrong_vertices[edge.first], wrong_vertices[edge.second]));
                }
            }
            ...
            for (const auto& edge : mwpm_final) {
                ...
                if (already_added.find(first_index) == already_added.end()) {
                    eulerian_graph_edges.emplace_back(first_index, second_index, sym_matrix[first_index][second_index]);
            ...
            std::unordered_map<Index, std::list<Index>> eulerian_adjacency_list = eulerian_graph.get_adjacency_list();
            ...
            do {
                ...
                for (auto vertex = eulerian_path.begin(); vertex != eulerian_path.end(); ++vertex) {
                ...
                if (!complete_tour) {
                    ...
                    do {
                        ...
                        for (auto vertex = eulerian_adjacency_list[next_vertex].begin();
                             vertex != eulerian_adjacency_list[next_vertex].end(); ++vertex) {
