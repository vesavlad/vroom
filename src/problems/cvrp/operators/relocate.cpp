...
namespace vroom
{
    namespace cvrp
    {
        ...
        void Relocate::compute_gain()
        {
            ...
            // rank t_rank.
            Gain t_gain = -utils::addition_cost(_input, m, s_route[s_rank], v, t_route, t_rank);
        ...
        bool Relocate::is_valid()
        {
            ...
            if (_sol_state.fwd_amounts[t_vehicle].empty()) {
                valid &= (_input.jobs[relocate_job_rank].amount <= _input.vehicles[t_vehicle].capacity);
            } else {
                valid &= (_sol_state.fwd_amounts[t_vehicle].back() + _input.jobs[relocate_job_rank].amount <=
        ...
        std::vector<Index> Relocate::addition_candidates() const { return {s_vehicle}; }

        std::vector<Index> Relocate::update_candidates() const { return {s_vehicle, t_vehicle}; }
