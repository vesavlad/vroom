...
namespace vroom
{
    namespace cvrp
    {
        ...
        void IntraRelocate::compute_gain()
        {
            ...
            Gain t_gain = -utils::addition_cost(_input, m, s_route[s_rank], v_target, t_route, new_rank);

            stored_gain   = _sol_state.node_gains[s_vehicle][s_rank] + t_gain;
            gain_computed = true;
        }

        bool IntraRelocate::is_valid() { return true; }
        ...
        std::vector<Index> IntraRelocate::addition_candidates() const { return {}; }

        std::vector<Index> IntraRelocate::update_candidates() const { return {s_vehicle}; }
