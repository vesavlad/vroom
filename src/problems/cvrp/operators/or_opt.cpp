    ...
    namespace cvrp
    {
        ...
        void OrOpt::compute_gain()
        {
            ...
            Gain reverse_edge_cost =
              static_cast<Gain>(m[s_index][after_s_index]) - static_cast<Gain>(m[after_s_index][s_index]);
            Gain reverse_t_gain = old_edge_cost + reverse_edge_cost - reverse_previous_cost - reverse_next_cost;

            normal_stored_gain   = _sol_state.edge_gains[s_vehicle][s_rank] + t_gain;
            reversed_stored_gain = _sol_state.edge_gains[s_vehicle][s_rank] + reverse_t_gain;
        ...
        bool OrOpt::is_valid()
        {
            ...
            if (_sol_state.fwd_amounts[t_vehicle].empty()) {
                valid &= (_input.jobs[current_job_rank].amount + _input.jobs[after_job_rank].amount <=
                          _input.vehicles[t_vehicle].capacity);
            } else {
                valid &= (_sol_state.fwd_amounts[t_vehicle].back() + _input.jobs[current_job_rank].amount +
        ...
        void OrOpt::apply()
        {
            t_route.insert(t_route.begin() + t_rank, s_route.begin() + s_rank, s_route.begin() + s_rank + 2);
            if (reverse_s_edge) {
        ...
        std::vector<Index> OrOpt::addition_candidates() const { return {s_vehicle}; }

        std::vector<Index> OrOpt::update_candidates() const { return {s_vehicle, t_vehicle}; }
