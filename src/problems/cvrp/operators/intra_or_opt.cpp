...
namespace vroom
{
    namespace cvrp
    {
        IntraOrOpt::IntraOrOpt(const Input&                input,
                               ...
                               Index                       s_rank,
                               Index                       t_rank)
            : Operator(input, sol_state, s_route, s_vehicle, s_rank, s_route, s_vehicle, t_rank), reverse_s_edge(false)
        {
        ...
        void IntraOrOpt::compute_gain()
        {
            ...
            Gain reverse_add_gain = old_edge_cost + reverse_edge_cost - reverse_previous_cost - reverse_next_cost;

            normal_stored_gain   = _sol_state.edge_gains[s_vehicle][s_rank] + add_gain;
            reversed_stored_gain = _sol_state.edge_gains[s_vehicle][s_rank] + reverse_add_gain;
        ...
        bool IntraOrOpt::is_valid() { return true; }
        ...
        std::vector<Index> IntraOrOpt::addition_candidates() const { return {}; }

        std::vector<Index> IntraOrOpt::update_candidates() const { return {s_vehicle}; }
