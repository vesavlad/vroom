...
namespace vroom
{
    namespace cvrp
    {
        ...
        bool ReverseTwoOpt::is_valid()
        {
            ...
            valid &= (_sol_state.fwd_amounts[s_vehicle][s_rank] + _sol_state.fwd_amounts[t_vehicle][t_rank] <=
                      _input.vehicles[s_vehicle].capacity);
            valid &= (_sol_state.bwd_amounts[t_vehicle][t_rank] + _sol_state.bwd_amounts[s_vehicle][s_rank] <=
        ...
        void ReverseTwoOpt::apply()
        {
            ...
            t_route.insert(t_route.begin(), s_route.rbegin(), s_route.rbegin() + nb_source);
            s_route.erase(s_route.begin() + s_rank + 1, s_route.end());
            s_route.insert(s_route.end(), t_route.rend() - t_rank - nb_source - 1, t_route.rend() - nb_source);
            t_route.erase(t_route.begin() + nb_source, t_route.begin() + nb_source + t_rank + 1);
        }

        std::vector<Index> ReverseTwoOpt::addition_candidates() const { return {s_vehicle, t_vehicle}; }

        std::vector<Index> ReverseTwoOpt::update_candidates() const { return {s_vehicle, t_vehicle}; }
