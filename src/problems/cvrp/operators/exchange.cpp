...
namespace vroom
{
    namespace cvrp
    {
        Exchange::Exchange(const Input&                input,
                           ...
                           Index                       t_vehicle,
                           Index                       t_rank)
            : Operator(input, sol_state, s_route, s_vehicle, s_rank, t_route, t_vehicle, t_rank)
        {
        ...
        void Exchange::compute_gain()
        {
            ...
            Gain s_gain = _sol_state.edge_costs_around_node[s_vehicle][s_rank] - new_previous_cost - new_next_cost;
            ...
            Gain t_gain = _sol_state.edge_costs_around_node[t_vehicle][t_rank] - new_previous_cost - new_next_cost;
        ...
        bool Exchange::is_valid()
        {
            ...
            valid &= (_sol_state.fwd_amounts[t_vehicle].back() - _input.jobs[t_job_rank].amount +
                        _input.jobs[s_job_rank].amount <=
                      _input.vehicles[t_vehicle].capacity);

            valid &= (_sol_state.fwd_amounts[s_vehicle].back() - _input.jobs[s_job_rank].amount +
                        _input.jobs[t_job_rank].amount <=
                      _input.vehicles[s_vehicle].capacity);

            return valid;
        }

        void Exchange::apply() { std::swap(s_route[s_rank], t_route[t_rank]); }

        std::vector<Index> Exchange::addition_candidates() const { return {s_vehicle, t_vehicle}; }

        std::vector<Index> Exchange::update_candidates() const { return {s_vehicle, t_vehicle}; }
