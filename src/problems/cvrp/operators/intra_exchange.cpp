...
namespace vroom
{
    namespace cvrp
    {
        IntraExchange::IntraExchange(const Input&                input,
                                     ...
                                     Index                       s_rank,
                                     Index                       t_rank)
            : Operator(input, sol_state, s_route, s_vehicle, s_rank, s_route, s_vehicle, t_rank)
        {
        ...
        void IntraExchange::compute_gain()
        {
            ...
            Gain t_gain = _sol_state.edge_costs_around_node[s_vehicle][t_rank] - new_previous_cost - new_next_cost;

            stored_gain   = s_gain + t_gain;
            gain_computed = true;
        }

        bool IntraExchange::is_valid() { return true; }

        void IntraExchange::apply() { std::swap(s_route[s_rank], t_route[t_rank]); }

        std::vector<Index> IntraExchange::addition_candidates() const { return {}; }

        std::vector<Index> IntraExchange::update_candidates() const { return {s_vehicle}; }
