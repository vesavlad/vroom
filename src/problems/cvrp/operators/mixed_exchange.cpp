...
namespace vroom
{
    namespace cvrp
    {
        MixedExchange::MixedExchange(const Input&                input,
                                     ...
                                     Index                       t_vehicle,
                                     Index                       t_rank)
            : Operator(input, sol_state, s_route, s_vehicle, s_rank, t_route, t_vehicle, t_rank), reverse_t_edge(false)
        {
        ...
        void MixedExchange::compute_gain()
        {
            ...
            normal_s_gain = _sol_state.edge_costs_around_node[s_vehicle][s_rank] - previous_cost - next_cost;

            Gain reverse_edge_cost =
              static_cast<Gain>(m[t_index][t_after_index]) - static_cast<Gain>(m[t_after_index][t_index]);
            reversed_s_gain = _sol_state.edge_costs_around_node[s_vehicle][s_rank] + reverse_edge_cost -
            ...
            t_gain = _sol_state.edge_costs_around_edge[t_vehicle][t_rank] - previous_cost - next_cost;
        ...
        bool MixedExchange::is_valid()
        {
            ...
            valid &= (_sol_state.fwd_amounts[s_vehicle].back() - _input.jobs[s_job_rank].amount +
                        _input.jobs[t_job_rank].amount + _input.jobs[t_after_job_rank].amount <=
                      _input.vehicles[s_vehicle].capacity);

            valid &= (_sol_state.fwd_amounts[t_vehicle].back() - _input.jobs[t_job_rank].amount -
        ...
        void MixedExchange::apply()
        {
            ...
            s_route.insert(s_route.begin() + s_rank + 1, t_route.begin() + t_rank + 1, t_route.begin() + t_rank + 2);
        ...
        std::vector<Index> MixedExchange::addition_candidates() const { return {s_vehicle, t_vehicle}; }

        std::vector<Index> MixedExchange::update_candidates() const { return {s_vehicle, t_vehicle}; }
