...
namespace vroom
{
    namespace cvrp
    {
        ...
        bool TwoOpt::is_valid()
        {
            ...
            valid &= (_sol_state.fwd_amounts[s_vehicle][s_rank] + _sol_state.bwd_amounts[t_vehicle][t_rank] <=
                      _input.vehicles[s_vehicle].capacity);
            valid &= (_sol_state.fwd_amounts[t_vehicle][t_rank] + _sol_state.bwd_amounts[s_vehicle][s_rank] <=
        ...
        void TwoOpt::apply()
        {
            ...
            t_route.insert(t_route.begin() + t_rank + 1, s_route.begin() + s_rank + 1, s_route.end());
            s_route.erase(s_route.begin() + s_rank + 1, s_route.end());
            s_route.insert(s_route.end(), t_route.begin() + t_rank + 1 + nb_source, t_route.end());
            t_route.erase(t_route.begin() + t_rank + 1 + nb_source, t_route.end());
        }

        std::vector<Index> TwoOpt::addition_candidates() const { return {s_vehicle, t_vehicle}; }

        std::vector<Index> TwoOpt::update_candidates() const { return {s_vehicle, t_vehicle}; }
